...
class SymbolMatcherMode(frontend.Mode):
    ...
    .. note:: This mode requires the document to be filled with
        :class:`pyqode.core.frontend.TextBlockUserData`, i.e. a
        :class:`pyqode.core.frontend.SyntaxHighlighter` must be installed on
        the editor instance.
    ...
    def _on_state_changed(self, state):  # pylint: disable=missing-docstring
        if state:
    ...
    def _create_decoration(self, pos, match=True):
        ...
        deco = frontend.TextDecoration(cursor, draw_order=10)
